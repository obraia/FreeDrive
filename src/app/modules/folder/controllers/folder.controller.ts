...
class FolderController extends BaseController<IFolder> {
  ...
  public async download(req: Request, res: Response) {
    ...
    try {
      ...
      const folders = await this.repository.findDeepChildren(ids.map((i) => new Types.ObjectId(i)));

      if (!folders.length) {
        throw new NotfoundException('Not found');
      }

      if (folders.every((f: any) => f.children.length === 0 && f.files.length === 0)) {
  ...
  private async _recursiveSearch(parent: Partial<IFolder> & { path: string }, folders = [] as any[]) {
    for (const child of parent.children!) {
      const path = `${parent.path}/${child.folderName}`;

      const [childFolder] = await this.repository.findDeepChildren([child._id]);

      folders.push({ folderName: childFolder.folderName, files: childFolder.files, path });

      if (childFolder.children) {
        await this._recursiveSearch({ children: childFolder.children, path }, folders);
      }
    }
